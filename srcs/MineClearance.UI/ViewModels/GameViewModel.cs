...
public sealed partial class GameViewModel : ObservableObject
{
    ...
    private void ShowHelp()
    {
        _toast.Show(
            "🎮 操作提示\n" +
            "首次点击后生成地雷, 确保首次点击的格子及周围格子尽量不会是地雷, 在此之前不允许右键操作\n" +
            "左键: 未打开格子打开, 数字格子在周围插旗数匹配时展开周围\n" +
            "右键: 数字格在周围未打开格子数匹配时一键插旗周围, 其他格循环标记 旗/问号/取消\n" +
            "按住左键/右键滑动: 持续操作\n" +
            "按住显示索引热键: 显示所有格子索引, 仅在等待开始时有效\n" +
            "首次点击后若启用首点复制索引, 将自动复制首次点击的格子索引到剪贴板\n" +
            "当场上没有可以被确定安全的格子需要被迫进行猜测时 (比如经典二选一情况), 游戏会确保你点击的格子不是地雷"
        );
    }
